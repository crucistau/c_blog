...
import xyz.crucistau.constants.RedisConst;
import xyz.crucistau.constants.SQLConst;
import xyz.crucistau.domain.dto.ArticleDTO;
...
import xyz.crucistau.domain.vo.*;
import xyz.crucistau.enums.*;
import xyz.crucistau.exceptions.FileUploadException;
import xyz.crucistau.mapper.*;
import xyz.crucistau.service.*;
import xyz.crucistau.utils.*;
...
public class ArticleServiceImpl extends ServiceImpl<ArticleMapper, Article> implements ArticleService {
    ...
    public PageVO<List<ArticleVO>> listAllArticle(Integer pageNum, Integer pageSize) {
        ...
        List<ArticleVO> articleVOS = list.stream().map(article -> {
            ...
            articleVO.setArticleCover(minioUrlUtils.convertUrl(articleVO.getArticleCover()));
            return articleVO;
        }).toList();
    ...
    public List<RecommendArticleVO> listRecommendArticle() {
        ...
        return articles.stream().map(article -> {
            RecommendArticleVO vo = article.asViewObject(RecommendArticleVO.class);
            vo.setArticleCover(minioUrlUtils.convertUrl(vo.getArticleCover()));
            return vo;
        }).toList();
    }
    ...
    public List<RandomArticleVO> listRandomArticle() {
        ...
        return randomArticles.stream()
                .map(article -> {
                    RandomArticleVO vo = article.asViewObject(RandomArticleVO.class);
                    vo.setArticleCover(minioUrlUtils.convertUrl(vo.getArticleCover()));
                    return vo;
                })
                .toList();
    ...
    public List<RelatedArticleVO> relatedArticleList(Integer categoryId, Integer articleId) {
        ...
        return articlesLimit5.stream().map(article -> {
            RelatedArticleVO vo = article.asViewObject(RelatedArticleVO.class);
            vo.setArticleCover(minioUrlUtils.convertUrl(vo.getArticleCover()));
            return vo;
        }).toList();
    }
    ...
    public List<TimeLineVO> listTimeLine() {
        ...
        return list.stream().map(article -> {
            TimeLineVO vo = article.asViewObject(TimeLineVO.class);
            vo.setArticleCover(minioUrlUtils.convertUrl(vo.getArticleCover()));
            return vo;
        }).toList();
    }
    ...
    public List<CategoryArticleVO> listCategoryArticle(Integer type, Long typeId) {
        ...
        return articles.stream().map(article -> article.asViewObject(CategoryArticleVO.class, item -> {
            ...
            // 替换 MinIO URL 为当前配置的 endpoint
            item.setArticleCover(minioUrlUtils.convertUrl(item.getArticleCover()));
        })).toList();
    }
    ...
    public List<ArticleListVO> listArticle() {
        ...
        if (!articleListVOS.isEmpty()) {
            articleListVOS.forEach(articleListVO -> {
                ...
                // 替换 MinIO URL 为当前配置的 endpoint
                articleListVO.setArticleCover(minioUrlUtils.convertUrl(articleListVO.getArticleCover()));
                // 查询文章标签
