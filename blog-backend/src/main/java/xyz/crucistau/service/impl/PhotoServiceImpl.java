...
import xyz.crucistau.enums.AlbumOrPhotoEnum;
import xyz.crucistau.enums.UploadEnum;
import xyz.crucistau.exceptions.FileUploadException;
import xyz.crucistau.mapper.PhotoMapper;
import xyz.crucistau.service.PhotoService;
...
import static xyz.crucistau.constants.SQLConst.LIMIT_ONE_SQL;
import static xyz.crucistau.constants.SQLConst.ORDER_BY_CREATE_TIME_DESC;
...
public class PhotoServiceImpl extends ServiceImpl<PhotoMapper, Photo> implements PhotoService {
    ...
    public PageVO<List<PhotoAndAlbumListVO>> getBackPhotoList(Long pageNum, Long pageSize, Long parentId) {
        ...
        for (Photo photo : page.getRecords()) {
            if (Objects.equals(photo.getType(), AlbumOrPhotoEnum.ALBUM.getCode())) {
                ...
                if (null != photoOne && StringUtils.isValidUrl(photoOne.getUrl())) {
                    page.getRecords().get(page.getRecords().indexOf(photo)).setUrl(minioUrlUtils.convertUrl(photoOne.getUrl()));
                }else{
        ...
        List<PhotoAndAlbumListVO> photoAndAlbumListVOS = page.getRecords().stream().map(photo -> {
            PhotoAndAlbumListVO vo = photo.asViewObject(PhotoAndAlbumListVO.class);
            vo.setUrl(minioUrlUtils.convertUrl(vo.getUrl()));
            return vo;
        }).toList();
